...
/** Tests unitaires de la classe {@link FichierSortieMapper}*/
public class FichierSortieMapperTest {
    ...
    @Test
    public void test_mapListesMetierToListeData(){
        ListesUtilitaires.getInstance(listeMontagnes, listeAventuriers,
                listeTresors, listeCartes);

        ArrayList<String> listeActual = FichierSortieMapper.mapListesMetierToListeData();
        String expected = "[C - 3 - 4, M - 1 - 0," +
                " M - 2 - 1, T - 0 - 3 - 2, T - 1 - 3 - 3, A - Lara - 1 - 1 - S - 0, A - Paul - 1 - 2 - S - 0]";
        assertEquals(expected, listeActual.toString());
    }
}
